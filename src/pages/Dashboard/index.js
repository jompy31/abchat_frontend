...
const loadCompanies = async () => {
  setLoading(true);
  try {
    // Verificar si companyId tiene un valor antes de intentar buscar los datos
    if (companyId) {
      const companiesList = await finding(companyId);
      setCompanyDueDate(moment(companiesList.dueDate).format("DD/MM/yyyy"));
    } else {
      console.log("No se encontró el ID de la empresa en localStorage.");
      // Puedes establecer un valor predeterminado o dejar el campo vacío
      setCompanyDueDate(null); // o cualquier valor predeterminado
    }
  } catch (e) {
